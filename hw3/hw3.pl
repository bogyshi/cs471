...
/* Problem 1 Answer: */
my_member(X,[X|_Y]).
my_member(X,[_X|Y]):-my_member(X,Y).
/* Problem 1 Test: */
...
/* Problem 3 Answer: */
is_increasing([X,Y]):-X<Y.
is_increasing([X,Y]):-X=Y.
is_increasing([X,Y|Z]):-X<Y,is_increasing([Y|Z]).
is_increasing([X,Y|Z]):-X=Y,is_increasing([Y|Z]).
/* Problem 3 Test: */
...
/* Problem 4 Answer: */
element_at(X,[X|_Z],0).
element_at(X,[_Z|Y],N):-R is N-1,element_at(X,Y,R).
/* Problem 4 Test: */
...
/* Problem 5 Answer: */
insert_at(_XX,[],-1,[]).
insert_at(V,X,0,Z):- insert_at(_XX,[V|X],-1,Z).
insert_at(_XX,[X|Y],-1,[X|Z]):-insert_at(_XY,Y,-1,Z).
insert_at(V,[X|Y],N,[X|Z]):- N>0, R is N-1, insert_at(V,Y,R,Z).
/* Problem 5 Test: */
...
/* Problem 6 Answer: */
zip([],[],[]).
zip([X|Y],[Z|W],[A|B]):- (X,Z)=A, zip(Y,W,B).
/* Problem 6 Test: */
...
zip2([],_X,[]).
zip2(_Y,[],[]).
zip2([X|Y],[Z|W],[A|B]):- (X,Z)=A, zip2(Y,W,B).
/* Problem 7 Test: */
...
/* Problem 8 Answer: */
merge([],[],[]).
merge([X|Y],[Z|W],[X|B]):- X>Z , merge(Y,[Z|W],B).
merge([X|Y],[Z|W],[X|B]):- X=Z , merge(Y,[Z|W],B).
merge([X|Y],[],[X|B]):-merge(Y,[],B).
merge([X|Y],[Z|W],[Z|B]):- Z>X , merge([X|Y],W,B).
merge([],[Z|W],[Z|B]):-merge([],W,B).
/* Problem 8 Test: */
...
/* Problem 11:
(From Learn Prolog NOW!) Binary trees are trees where all internal nodes have exactly two children. The smallest binary trees consist of only one leaf node. We will represent leaf nodes as leaf(Label).
For instance, leaf(3) and leaf(7) are leaf nodes, and therefore small binary trees. Given two binary trees B1 and B2 we can combine them into one binary tree using the predicate tree: tree(B1,B2).
So, from the leaves leaf(1) and leaf(2) we can build the binary tree tree(leaf(1), leaf(2)). And from the binary trees tree(leaf(1), leaf(2)) and leaf(4) we can build the binary
...
/* Problem 11 Answer: */
isBinaryTree(leaf(_X)).
isBinaryTree(tree(X,Y)):-isBinaryTree(X),isBinaryTree(Y).
% isBinaryTree(X):- X=tree(Y,Z),isBinaryTree(Y),isBinaryTree(Z). %
/* Problem 11 Test: */
